...
export function parseSessionMessages(filePath) {
    const out = [];
    try {
        ...
        for (const line of lines) {
            ...
            if (msg && (msg.type === 'user_input' || msg.type === 'agent_message')) {
                if (msg.type === 'user_input' && msg.text)
                    out.push({ role: 'user', text: String(msg.text) });
                if (msg.type === 'agent_message' && msg.message)
                    out.push({ role: 'assistant', text: String(msg.message) });
            }
        }
    }
    catch { }
