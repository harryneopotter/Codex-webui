...
export class CodexService extends EventEmitter {
  ...
  public sendUserInput(text: string) {
    if (!this.codexProc) return;
  ...
  private handleStdout(chunk: string) {
    ...
    for (const raw of lines) {
      ...
      if (type === 'session_configured') {
        ...
        try { this.codexProc?.stdin?.write(JSON.stringify(ctl) + '\n'); } catch {}
        this.emit('broadcast', 'system', { text: 'Codex session configured' });
        if (this.lastResumePath) this.recordResume(this.lastResumePath);
        this.emit('status_update');
      }
